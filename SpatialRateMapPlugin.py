...
import functools #  for stripAxes decorator
...
class SpatialRateMap(ManualClusteringView):
    ...
    def plotRateMap(self):
        self.canvas.ax.clear()
        self.RateMapMaker.makeRateMap(self.cluster_ids[0], ax=self.canvas.ax)
        self.plot_type = "ratemap"
        self.canvas.update()
...
class MapCalcsGeneric(object):
    ...
    * speed vs rate plots
    ...
    def makeRateMap(self, cluster, ax, **kwargs):
    ...
    def makeSpikePathPlot(self, cluster, ax, **kwargs):
        ax.plot(self.xy[0], self.xy[1], c=[1, 1, 1])
        ax.set_aspect('equal')
        ax.invert_yaxis()
    ...
    def makeHDPlot(self, cluster, ax, **kwargs):
        ...
        r = rmap[0]
        r = np.insert(r, -1, r[0])
        w = np.mean(np.diff(theta))
        ax.bar(theta, r, width=w)
        ax.set_aspect('equal')
        plt.setp(ax.get_xticklabels(), visible=False)
        plt.setp(ax.get_yticklabels(), visible=False)
        ax.axes.get_xaxis().set_visible(False)
